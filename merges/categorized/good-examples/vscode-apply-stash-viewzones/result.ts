...
export class CommandViewZoneFactory extends Disposable {
	...
	public createWidget(viewZoneChangeAccessor: IViewZoneChangeAccessor, lineNumber: number, items: IObservable<ICommandViewZoneItem[]>, viewZoneIdsToCleanUp: string[]): IDisposable {
		const layoutInfo = this._getLayoutInfo();
		return new CommandViewZone(
			this._editor,
...
export interface ICommandViewZoneItem {
	text: string;
	tooltip?: string;
	action?: () => Promise<void>;
}

class CommandViewZone extends FixedZoneWidget {
	...
	constructor(
		...
		className: string,
		items: IObservable<ICommandViewZoneItem[]>,
	...
	private setState(items: ICommandViewZoneItem[]) {
...
export class MergeEditorCommandsSource {
	constructor(
	...
	private getItemsInput(inputNumber: 1 | 2): IObservable<ICommandViewZoneItem[]> {
		return derived('items', reader => {
			...
			const result: ICommandViewZoneItem[] = [];
			...
			if (!state.manualResolution && !state.isInputIncluded(inputNumber)) {
				result.push(
...
>>>>>>> theirs
		...
		const stateToggles: ICommandViewZoneItem[] = [];
		if (state.input1) {
...
function command(title: string, action: () => Promise<void>, tooltip?: string): ICommandViewZoneItem {
	return {
		text: title,
		action,
		tooltip,
